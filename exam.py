...
#     subjects = json.loads(data[0][5])
#     conn.close()
#     return data[0][5], data[0][6], subjects.get('English', '')



# print(get_exams_data())
...
# Connect to the database
db_path = "student.db"  # Ensure this is the correct path
conn = sqlite3.connect(db_path)
cursor = conn.cursor()
...
#     # Add column only if it doesn't exist (SQLite doesn't support IF NOT EXISTS for columns directly)
#     try:
#         cursor.execute("ALTER TABLE marks ADD COLUMN average REAL;")

...
#     all_subject_names = set() # To keep track of every unique subject found
#     processed_records = []

#     for row in rows:
#         item = dict(row)
#         # Parse JSON
#         if isinstance(item['marks_json'], str):
#             marks_dict = json.loads(item['marks_json'])
#             item['marks_json'] = marks_dict
#             # Add these subjects to our master set
#             all_subject_names.update(marks_dict.keys())
#         else:
#             item['marks_json'] = {}
...
#     conn.close()
#     # Sort subjects alphabetically for a clean table
#     return processed_records, sorted(list(all_subject_names))